...
public class Combustion extends CombustionAbility implements SubAbility {
	...
	private String getHexColor(long min, long max) {
	...
	private void releaseCombustion() {
		...
		if (advancementSteps % sonicBoomFrequency == 0) {
			...
			location.getWorld().playSound(location,Sound.ENTITY_FIREWORK_ROCKET_BLAST,3,0);
		}
	...
	private void explode(Location explosion, boolean isWater, boolean isEntity, @Nullable Entity entity) {
		...
		if (isWater) {
			...
			explosion.getWorld().spawnParticle(Particle.EXPLOSION_LARGE, explosion, (int) explosionRadius * 200, explosionRadius, explosionRadius*1.5, explosionRadius, 0, null, true);
			// For when the core is 1.21+ -> explosion.getWorld().spawnParticle(Particle.GUST_EMITTER_LARGE, explosion.add(0, 1.5, 0), 1, 0, 0, 0, 0,null,true);
	...
	/**
	 * @param clicked Sets whether the player clicked during the ability, used for controlled explosion
	 */
	public void setClicked(boolean clicked) { this.clicked=clicked; }

	/**
	 * Ignored if punishPlayer is false.
	 * @param damaged Sets whether the player got damaged during the ability, used for punishing the player.
	 */
